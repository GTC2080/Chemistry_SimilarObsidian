...
- **Backend boundary**: core note IO, vault root validation, vault file reads, PDF file hashing, search, tags, backlinks, graph, file tree, product/database normalization, publishing compile-plan rules, AI embedding cache retrieval, embedding refresh job planning, and study/session storage/statistics are owned by `kernel/`; Tauri Rust keeps command registration, bridge wiring, platform glue, external network calls, external process launching, and thin orchestration

## Recent Updates

- **v1.0.6-dev** — publishing compile-plan rules are now kernel-owned: `compile_markdown_to_pdf` asks `kernel_build_paper_compile_plan_json(...)` for paper template args, CSL/bibliography path trimming, and Pandoc resource-path parent extraction/deduping/joining; Rust `compiler.rs` only writes temporary files, selects the platform separator, and launches Pandoc/XeLaTeX
...
- **v1.0.6-dev** — enriched graph adjacency is now kernel-owned: `get_enriched_graph_data` calls `kernel_query_enriched_graph_json(...)`, so `neighbors` and `linkPairs` come directly from the C++ kernel; Rust no longer rebuilds the graph adjacency map or directed pair strings
- **v1.0.6-dev** — changed-entry `NoteInfo` filtering is now kernel-owned: `scan_changed_entries` calls `kernel_query_changed_notes(...)`, so Markdown path filtering, normalization, deduping, catalog matching, and changed-path result ordering are handled by the C++ kernel; Rust no longer pulls the full catalog and rebuilds incremental results with `HashSet` / `HashMap`
- **v1.0.6-dev** — deleted-entry embedding cleanup is now kernel-owned: `remove_deleted_entries` calls `kernel_delete_changed_ai_embedding_notes(...)`, so changed-path filtering/deduping, embedding metadata deletion, and delete counting all run in the C++ kernel; the Tauri bridge no longer keeps a single-note `delete_ai_embedding_note` wrapper
- **v1.0.6-dev** — AI RAG content hydration is now kernel-owned: `ask_vault` calls `kernel_build_ai_rag_context_from_changed_note_paths(...)`, so candidate note Markdown filtering, normalization, deduping, note-content reads, missing-note skipping, blank-note skipping, display-name derivation, block numbering, and truncation all run in the C++ kernel; Rust `cmd_ai.rs` no longer loops over note reads or builds the context
- **v1.0.6-dev** — indexed Markdown content reads are now kernel-owned: `read_note_indexed_content` calls `kernel_read_first_changed_markdown_note_content(...)`, so note-id Markdown filtering, normalization, deduping, and first-candidate content reads all run in the C++ kernel; Rust `cmd_media.rs` no longer combines a path filter with `read_note_by_rel_path`
...
- **Kernel embedding cache migration (v1.0.6-dev)**: semantic search, related notes, RAG candidate retrieval, and RAG content hydration now call kernel surfaces over `ai_embedding_cache` / note IO; Rust only schedules embedding requests and maps command DTOs.
