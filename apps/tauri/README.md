...
- **v1.0.6-dev** — deleted-entry embedding cleanup 继续内核化：`remove_deleted_entries` 改走 `kernel_delete_changed_ai_embedding_notes(...)`，changed path 过滤/去重、embedding metadata 删除和删除计数都由 C++ kernel 完成；Tauri bridge 删除了单条 `delete_ai_embedding_note` wrapper。
- **v1.0.6-dev** — AI RAG 内容读取继续内核化：`ask_vault` 改走 `kernel_build_ai_rag_context_from_changed_note_paths(...)`，候选 note id 的 Markdown 过滤/归一/去重、正文读取、缺失 note 跳过、空白 note 跳过、展示名派生、RAG block 编号和截断都由 C++ kernel 负责；Rust `cmd_ai.rs` 不再循环读取 note 或拼上下文。
- **v1.0.6-dev** — indexed Markdown 内容读取继续内核化：`read_note_indexed_content` 改走 `kernel_read_first_changed_markdown_note_content(...)`，note id 的 Markdown 过滤/归一/去重与首个候选正文读取都由 C++ kernel 完成；Rust `cmd_media.rs` 不再组合 path filter 与 `read_note_by_rel_path`。
- **v1.0.6-dev** — study/session 存储全面迁入 C++ kernel：`study_session_start` / `study_session_tick` / `study_session_end`、`study_stats_query`、`truth_state_from_study`、`get_heatmap_cells` 都通过 sealed bridge 调用 kernel；Rust `src/db/`、`DbState`、`rusqlite` 依赖与 legacy study SQLite 兼容层已删除。
- **v1.0.6-dev** — legacy embedding refresh job 全面迁入 C++ kernel：`index_vault_content` / `index_changed_entries` / `rebuild_vector_index` 改走 `kernel_prepare_ai_embedding_refresh_jobs(...)` 与 `kernel_prepare_changed_ai_embedding_refresh_jobs(...)`；kernel 负责 note catalog 读取、ignored root 过滤、changed path refresh、timestamp 比较、正文读取、indexability 判断和 metadata upsert，Rust 只调外部 embedding API 并写回 vector。
- **v1.0.6-dev** — legacy `index.db` 退出 note/vault/study 主链路：`init_vault` 只打开 sealed kernel session，不再初始化 Rust `DbState` 或创建 vault 根目录下的 `index.db`；study/session 数据现在写入 kernel storage schema 的 `study_sessions` 表。
...
- **v1.0.6-dev** — AI embedding indexing 预判继续内核化：正文是否值得进入 embedding 队列由 kernel refresh job planner 判定；Rust 不再用 `content.trim().is_empty()` 复制空白文本与截断后空文本规则。
...
- **v1.0.6-dev** — AI RAG context shape 继续内核化：RAG note header、1-based 编号、空白 note 跳过、note 分隔符和每条 note 的 Unicode 字符截断由 `kernel_build_ai_rag_context(...)` 提供；Rust `build_rag_context` 只调用 sealed bridge，不再拼接 note 上下文格式。
...
- **v1.0.6-dev** — database grid 列类型归一化继续内核化：`normalize_database` 的 `text` / `number` / `select` / `tags` 白名单与未知类型回退规则改走 `kernel_normalize_database_column_type(...)`；这一规则现在由 `kernel_normalize_database_json(...)` 作为完整 payload 归一化的一部分统一调用。
- **v1.0.6-dev** — study stats / truth / heatmap 从“Rust 读 SQLite + kernel 算规则”升级为“kernel 读写与聚合”：session 表、active seconds 写入、today/week/daily/folder ranking、TruthState、26x7 heatmap JSON 全部由 kernel handle-bound API 提供，Rust command 只透传参数和返回 JSON。
- **v1.0.6-dev** — AI RAG 笔记读取继续瘦身：`ask_vault` 的候选 note id 直接交给 `kernel_build_ai_rag_context_from_changed_note_paths(...)`，路径过滤、正文读取、缺失/空白跳过和上下文拼接都在 kernel 内完成。
- **v1.0.6-dev** — 内容/文件工作流继续收口到 C++ sealed kernel：`scan_vault`、`scan_changed_entries` 的笔记元数据统一来自 kernel note catalog，`index_vault_content` / `index_changed_entries` 的待 embedding job 由 kernel 直接准备，正文与 media 文本读取走 kernel note read surface；全量 note catalog 与 `build_file_tree` 的 ignored root 过滤分别由 `kernel_query_notes_filtered(...)`、`kernel_query_file_tree_filtered(...)` 提供，增量 changed-entry 的 Markdown 路径归一/过滤/去重由 `kernel_filter_changed_markdown_paths(...)` 提供。Tauri Rust 只保留命令编排、embedding 网络请求和后台任务调度。
...
- `scan_vault` -> `kernel_query_notes(...)` / `kernel_query_notes_filtered(...)`
- `scan_changed_entries` -> `kernel_query_changed_notes(...)`
...
- `index_vault_content` / `rebuild_vector_index` embedding refresh jobs -> `kernel_prepare_ai_embedding_refresh_jobs(...)`
- `index_changed_entries` changed embedding refresh jobs -> `kernel_prepare_changed_ai_embedding_refresh_jobs(...)`
- embedding refresh job planning owns note catalog lookup, ignored root filtering, changed-path catalog refresh, timestamp comparison, note reads, indexability checks, and metadata upsert
...
- `read_note` -> `kernel_read_note(...)`
- `read_note_indexed_content` -> `kernel_read_first_changed_markdown_note_content(...)`
- `read_binary_file` / `read_pdf_file` -> `kernel_read_vault_file(...)`
- `ask_vault` RAG note content -> `kernel_build_ai_rag_context_from_changed_note_paths(...)`
- `remove_deleted_entries` -> `kernel_delete_changed_ai_embedding_notes(...)`
...
AI embedding cache 的 refresh job planning、metadata、timestamp、vector BLOB、clear/delete 与 semantic top-k 检索由 kernel `ai_embedding_cache` surface 拥有；Rust 不再创建或读取本地 embedding SQL 表，也不再维护本地语义向量缓存。
AI embedding cache metadata 的刷新决策、内容读取、indexability 判断和 metadata upsert 在 refresh job planner 内完成，避免 `cmd_vault.rs` 保留本地 timestamp 比较、note read 循环或 cache 写入循环。
...
- `get_graph_data` -> `kernel_query_graph(...)`
- `get_enriched_graph_data` -> `kernel_query_enriched_graph_json(...)`
...
`get_enriched_graph_data` 的双向 `neighbors` 与 `linkPairs` 也由 kernel 从同一个 graph record 派生；Tauri Rust 只反序列化 kernel JSON 并返回 command DTO。
...
- legacy embedding BLOB codec -> `kernel_serialize_ai_embedding_blob(...)` + `kernel_parse_ai_embedding_blob(...)`
...
- `ask_vault` hydrated RAG context -> `kernel_build_ai_rag_context_from_changed_note_paths(...)`
- `normalize_database` payload normalization -> `kernel_normalize_database_json(...)`
- `study_session_start` / `study_session_tick` / `study_session_end` -> kernel-owned `study_sessions` storage
- `study_stats_query` / `truth_state_from_study` / `get_heatmap_cells` -> kernel-owned study query JSON

TRUTH_SYSTEM 经验归因规则、truth award reason key、AI 语义上下文裁剪规则、embedding 输入归一化、embedding refresh job planning、embedding cache key、legacy BLOB codec 规则、RAG note context shape 和 database grid payload 归一化由 kernel 提供；sealed C++ bridge 负责释放 kernel-owned truth diff result / semantic/embedding/RAG/database context buffer，并把结果转成 JSON/text/binary；Rust command 只映射 DTO 和中文展示文案。
...
- C++ kernel 追踪自由基、单体与 0/1/2 阶矩，Rust command 只做 sealed bridge 调用并返回 `time / conversion / Mn / PDI`
- 图表区显示两张实时曲线：
  - `Conversion vs Time`
...
│   ├── cmd_study.rs    # 学习时间轴命令壳（session/stats/truth/heatmap 由 kernel 提供）
