...
- **v1.0.6-dev** — 产品计算面继续内核化：`compute_truth_diff` 改为桥接 `kernel_compute_truth_diff(...)`，TRUTH_SYSTEM 经验归因、代码块语言增量和分子编辑行数规则由 C++ sealed kernel 统一提供；Tauri Rust 只保留 DTO 映射和中文 reason 文案。
...
TRUTH_SYSTEM 经验归因规则由 kernel 提供；Rust command 只映射 DTO 和中文展示文案。
