...
TRUTH_SYSTEM 经验归因规则、truth award reason key、AI 语义上下文裁剪规则、embedding 输入归一化规则和 RAG note context shape 由 kernel 提供；sealed C++ bridge 负责释放 kernel-owned truth diff result / semantic/embedding/RAG context buffer，并把结果转成 JSON/text；Rust command 只映射 DTO 和中文展示文案。
AI/product 文本边界默认值、embedding 输入归一化和 RAG 截断规则由 kernel 提供；Rust AI 模块只负责 HTTP 客户端、stream 解析、embedding 缓存和 IPC 发送，不再持有 semantic context/RAG/embedding 输入长度真相。
