...
- **v1.0.6-dev** — AI RAG context 空白 note 过滤继续内核化：`ask_vault` 读到候选正文后不再用 Rust `.trim().is_empty()` 判定是否跳过；空白内容跳过、RAG block 连续编号和每条 note 截断统一由 `kernel_build_ai_rag_context_from_note_paths(...)` 负责。
...
RAG note context shape 与 note display name derivation 由 kernel 提供；Rust AI 模块不再持有 note header、空白 note 跳过、编号、分隔符、展示名派生或每条 note 的截断规则。
