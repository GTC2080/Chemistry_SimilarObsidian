...
- **v1.0.6-dev** — 对称性原子解析与渲染几何继续内核化：`XYZ` / `PDB` / simple `CIF` 原子解析、元素归一化、原子质量表和 CIF 分数坐标转换由 `kernel_parse_symmetry_atoms_text(...)` 提供；旋转轴端点与镜像平面顶点由 `kernel_build_symmetry_render_geometry(...)` 提供；Rust 只继续保留候选轴/镜面搜索与 DTO 映射。
...
Rust `symmetry/` 当前仍保留候选旋转轴、镜像平面搜索和 3D viewer DTO 映射；后续迁移批次继续从这里削薄。
