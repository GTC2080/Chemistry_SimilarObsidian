...
TRUTH_SYSTEM 经验归因规则、truth award reason key、AI 语义上下文裁剪规则、embedding 输入归一化与 indexability 预判规则、embedding cache key 与 legacy embedding BLOB codec 规则、RAG note context shape 和 database grid 列类型归一化由 kernel 提供；sealed C++ bridge 负责释放 kernel-owned truth diff result / semantic/embedding/RAG context buffer，并把结果转成 JSON/text/binary；Rust command 只映射 DTO 和中文展示文案。
