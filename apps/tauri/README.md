...
- **v1.0.6-dev** — AI/product 文本边界继续内核化：`get_related_notes_raw` 的 semantic context 最小字节阈值、RAG 单笔记截断长度、embedding 输入截断长度分别通过 `kernel_get_semantic_context_min_bytes(...)`、`kernel_get_rag_context_per_note_char_limit(...)`、`kernel_get_embedding_text_char_limit(...)` 提供；Rust 只保留网络请求、缓存和 IPC 编排。
...
TRUTH_SYSTEM 经验归因规则、truth award reason key 和 AI 语义上下文裁剪规则由 kernel 提供；sealed C++ bridge 负责释放 kernel-owned truth diff result / semantic context buffer，并把结果转成 JSON/text；Rust command 只映射 DTO 和中文展示文案。
AI/product 文本边界默认值由 kernel 提供；Rust AI 模块只负责 HTTP 客户端、stream 解析、embedding 缓存和 IPC 发送，不再持有 semantic context/RAG/embedding 输入长度真相。
