...
- **v1.0.6-dev** — 化学无状态计算继续内核化：高分子动力学、化学计量、波谱解析、分子预览、逆合成 mock pathway 规则均通过 `kernel/` C ABI 提供；retrosynthesis / spectroscopy / molecular preview 的 kernel-owned 结果由 sealed C++ bridge 序列化为 JSON。Tauri Rust 只保留 PubChem HTTP 查询、命令 DTO 映射和 localized error 映射，不再保留 retrosynthesis C ABI mirror / unsafe 拷贝循环。
...
`parse_spectroscopy` 与 `read_molecular_preview` 的扩展名支持范围、CSV/JDX 解析和 PDB/XYZ/CIF 预览构造由 kernel compute ABI 判定；sealed C++ bridge 负责 kernel result -> JSON，Tauri Rust 只负责通过 `kernel_read_note(...)` 读取文本、调度阻塞计算和映射中文命令错误。

`recalculate_stoichiometry` 的空表行为、参考行选择、数值归一和依赖行传播由 kernel 判定；Tauri Rust 只保留表格 DTO 的字段映射。
...
TRUTH_SYSTEM 经验归因规则和 AI 语义上下文裁剪规则由 kernel 提供；Rust command 只映射 DTO、中文展示文案和 kernel-owned buffer。
