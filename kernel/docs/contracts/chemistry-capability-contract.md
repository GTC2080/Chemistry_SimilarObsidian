...
retrosynthesis pathway mock, spectroscopy text parser, and molecular preview
builder are chemistry compute/read surfaces, not Track 5 persisted capability
surfaces.
...
- `kernel_generate_mock_retrosynthesis(target_smiles, depth, out_tree)`
...
- the retrosynthesis surface returns a deterministic pathway tree derived only
  from caller-provided target SMILES and depth
...
Frozen retrosynthesis rules:

- target SMILES are normalized by removing whitespace before expansion
- empty targets return `KERNEL_ERROR_INVALID_ARGUMENT`
- depth is clamped to `1..4`
- target and precursor ids are generated by the kernel and use the `retro_`
  prefix
- `C(=O)N` targets emit `Amide Coupling`
- `C(=O)O` targets emit `Fischer Esterification`
- `Br`, `I`, or `B(` targets emit `Suzuki-Miyaura Coupling`
- unknown targets emit `Generic Bond Disconnection`
- only precursors with role `reactant` are recursively expanded
- invalid calls leave the output tree empty
