// Reason: Own truth award computation away from the product public ABI wrapper.
...
kernel_status compute_truth_diff(
  ...
  if (prev_content.empty() || curr_content.empty()) {
    return kernel::core::make_status(KERNEL_OK);
  }

  std::vector<TruthAwardDraft> awards;
  if (curr_content.size() > prev_content.size()) {
    const std::size_t delta = curr_content.size() - prev_content.size();
    if (delta > 10) {
      ...
      if (char_exp > 0) {
        awards.push_back(TruthAwardDraft{
            route_truth_attribute_by_extension(file_extension),
            char_exp,
  ...
  const std::set<std::string> new_blocks = extract_code_languages(curr_content);
  const std::set<std::string> old_blocks = extract_code_languages(prev_content);
  ...
  if (lower_ext == "mol" || lower_ext == "chemdraw") {
    const std::size_t prev_lines = line_count(prev_content);
    const std::size_t curr_lines = line_count(curr_content);
